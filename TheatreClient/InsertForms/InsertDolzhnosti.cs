...
{
    public partial class InsertDolzhnosti : MetroFramework.Forms.MetroForm
    {
        public InsertDolzhnosti(int _id)
        {
            InitializeComponent();
            id = _id;
            if (_id < 0)
        ...
        private void btnAction(object sender, EventArgs e)
        {
            try
            {
                ...
                switch (btn)
                {
                    case "Правка":
                        request =
                            "Update dolzhnosti set " +
                            "NameDolzhnosti= '" + nameText.Text + "'," +
                            "ZP= '" + ZPText.Text + "' " +
                            "Where id_dolzhnosti=" + id;
                        break;
                    case "Добавить":
                        request =
        ...
        void LoadString()
        {
            try
            {
                ...
                MySqlDataAdapter da = new MySqlDataAdapter
                    ...
                    "Where id_dolzhnosti=" + id, dbc);
                DataTable dt = new DataTable();
                da.Fill(dt);
                dbc.Close();
                nameText.Text = dt.Rows[0][1].ToString();
                ZPText.Text = dt.Rows[0][2].ToString();
            }
            catch { MessageBox.Show("Ошибка подключения к серверу. Попробуйте позже.", "TheatreClient", MessageBoxButtons.OK, MessageBoxIcon.Error); this.Close(); }
        }
    }
